(function() {
  ...
  function rowFacts(r){
    ...
    // Named people the nightly job found worth meeting at this event.
    const meet=(((r.targets_json||{}).people)||[]).length;
    if(meet)f.push(meet+' '+(meet===1?'person':'people')+' to meet');
    const line=f.join(' · ');
  ...
  function renderCalendar(rs) {
    ...
    trips.forEach(t=>{
      ...
      // The nightly job now caches named people worth meeting at each event
      // (targets_json). That only ever showed inside the event modal, so the
      // work was invisible from the board -- say it on the commitment itself.
      if(commit.length)html+='<ul class="ac-mini">'+commit.map(e=>{
        const n=(((e.targets_json||{}).people)||[]).length;
        return '<li>'+nameLink(e,'span')+' <span class="ac-where">'+esc(e.why)+(n?' · '+n+' '+(n===1?'person':'people')+' to meet':'')+'</span></li>';}).join('')+'</ul>';
