...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* Solo trip (nothing to batch) — a quiet note + one-click area search. */
    .trip-nonear {{ display: flex; align-items: center; flex-wrap: wrap; gap: 10px; margin: 4px 0 2px; }}
    .trip-nonear-note {{ font-size: 0.8rem; color: var(--ab-fg-3); font-style: italic; }}
    .trip-find-near {{ font-size: 0.78rem; padding: 5px 12px; }}
...
(function () {{
  ...
  function quickBarHtml(rec) {{
    ...
    bStage.push('<button type="button" class="qa qa-neg' + (has('Rejected') ? ' on' : '') + '" data-qa="rejected" title="Rejected to speak — the organizer passed. Angela can flag it so the team can opt to just attend.">' + (has('Rejected') ? '✓ Rejected' : 'Rejected to speak') + '</button>');
  ...
  function editFormHtml(rec) {{
    ...
    // Contact info stays for private events — it's part of the POC.
    ...
    if (!priv) h += ef('Post-mortem (ROI: contacts · meetings · sales vs cost)', ta('postmortem', rec.postmortem, 2));
  ...
  function openEventModal(rec) {{
    ...
    if (rec.is_private === true) badges.push('<span class="badge badge-private">Private</span>');
    ...
    var pocHtml = (contactBits.length ? field('Point of contact', contactBits.join(' · '), true) : '') +
                  field('Contact info', rec.contact_info);
    if (rec.is_private) {{
      ...
      var grid = '';
      grid += field('Deadline', (window.isStaleDeadline && window.isStaleDeadline(rec.deadline)) ? '' : rec.deadline);
  ...
  ready(function () {{
    ...
    function cardStatusLine(ev, st) {{
      ...
      if (stages.indexOf('Booked') !== -1) {{
        ...
        bits.push({{ p: 3, h: '<span class="st-bit"><span class="st-dot st-no"></span>Rejected to speak' + (speaker ? ' \\u2014 ' + escapeHtml(speaker) : '') + '</span>' }});
    ...
    function _whatsNewItems() {{
      ...
      items = items.filter(function (w) {{ return w.mention || _onRadar(w.kind, w.key); }});
    ...
    function renderMyEvents() {{
      ...
      function rowHtml(it) {{
        ...
        // so no brief button (see `briefable`).
        var brief = (it._past || !it.briefable) ? '' :
    ...
    function renderPlanAhead() {{
      ...
      if (clusters.length) {{
        ...
        function renderCluster(cl) {{
          ...
          // Nothing else tracked near this trip. Rather than a dead end, offer a
          // one-click AI search seeded with WHERE + WHEN — so a solo trip like
      ...
      // "Find events near <city>" on a solo trip → jump to the Events grid and
      // open the AI search pre-seeded with that trip's place + quarter. (The
      // search panel injects into the grid, which is hidden on Plan Ahead, so
      // switch views first, then scroll it into view.)
      host.querySelectorAll('[data-plan-near]').forEach(function (btn) {{
        btn.addEventListener('click', function () {{
