...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* Rejected — same red as the card's status dot, so "they passed" reads the
    ...
    /* "Also from this organiser" — the umbrella's members, or the family a
    ...
    /* The row is a wrapping flex line, so an editor dropped into it becomes a
       flex ITEM and gets squeezed into whatever space is left — a two-inch
    ...
    /* Status mark: always on, never hover-revealed — it IS the card's state. */
    .ops-stage-ico {{
      display: inline-flex; align-items: center; justify-content: center;
    ...
    /* The follow-up count control: + above, - below, in a stack UNDER the pill.
       ...
       positioned outside the parent's box — and its padding-top forms an
    ...
    .qa-fu-ctl {{
      ...
      padding-top: 7px;                 /* the bridge — do not turn into margin */
...
(function () {{
  ...
  // provided results" DOES say something ("Paid") — drop only the dead clause
  ...
  function briefClause(v) {{
    ...
    for (var i = 0; i < parts.length && picks.length < 4; i++) {{
      ...
      // "no guaranteed 1:1 meetings" is the absence of a feature — listing it
    ...
    // A meeting app is not a "format", and neither is being invite-only — the
    ...
    // silently dropped every plural clause — "keynotes; roundtables; expo floor"
    ...
    // broken English there — it goes through "Includes" instead.
    ...
    // Lead with the plainest one — insider shorthand ("Day 0") reads badly first.
    chosen.sort(function (a, b) {{
      var jarg = function (x) {{ return /\\bday\s*0\\b/i.test(x) ? 1 : 0; }};
      return jarg(a) - jarg(b);
    }});
    // Commas only — the last one becomes "and" below.
    var pick = chosen.join(', ');
    // Brief is the point — this rides at the END of the overview, so it has to
    // land in one readable breath rather than run on for three lines.
    if (pick.length > 110) pick = pick.slice(0, 107).replace(/[\s,]\S*$/, '') + '…';
    ...
    // Don't lowercase a brand — "Reuters Events app" must not become "reuters".
    ...
    // ("Reuters Events", "One-to-One Meetings") — those keep their capitals.
    // Everything else is an ordinary noun phrase and gets lowercased. Applied
    // per clause, so "…, Full access to keynotes" is fixed mid-sentence too,
    ...
    if (article) {{
      ...
      var _cut = _lst.search(/\s(?:for|with|via|in|on|at|to|during|across|from)\s|,/i);   // NOT 'and' — it's a conjunction, so the head noun continues past it
      var _head = (_cut > 0 ? _lst.slice(0, _cut) : _lst).trim().split(/\s+/).pop()
                    .replace(/[^A-Za-z-]+$/, '');
      // A gerund is a mass noun — "a matchmaking" / "a networking" are wrong.
  ...
  function quickBarHtml(rec) {{
    ...
    // top-right, as the same star and eye icons the card face uses — one place
  ...
  function wireQuickBar(rec) {{
    ...
    // and — for the follow-up pill — the button you just clicked survives to be
    ...
    function _qaLightRefresh(rec, qa) {{
      if (qa === 'followed-up' || qa === 'followed-up-minus') {{
        // Nothing STRUCTURAL changes when a chase is logged — only the count and
        // whether there's a quick entry left to undo. Patch those two in place:
        // replacing the node would drop the :hover that is revealing the "−",
        ...
        if (_pill) {{
          ...
          if (_n && !_pill.querySelector('.qa-tick')) {{
            _pill.insertAdjacentHTML('afterbegin', '<span class="qa-tick" aria-hidden="true">\\u2713</span>');
          }} else if (!_n) {{
            var _tk = _pill.querySelector('.qa-tick');
            if (_tk) _tk.remove();
          }}
        }}
      ...
      // A stage toggle DOES change the row's shape — an outcome retires and
      // locks the in-flight steps — so rebuild just the bar, not the modal.
    ...
    window.__qaClick = function (e) {{
      ...
      // Every click used to rebuild the WHOLE modal — 50ms of synchronous work
      // plus a chat re-render — which is why the pipeline felt sluggish and why
      ...
      _qaFullTimer = setTimeout(function () {{
        ...
        openEventModal(rec);
        // openEventModal rewrites $body, so the old node is stale — re-query.
  ...
  function openEventModal(rec) {{
    ...
    if (_fuMine) {{
      ...
      // noise: one bare entry "Jul 27 · Reached out" under "Followed up Jul 27"
      // says one thing twice (Hurley 2026-07-30). It comes back as soon as
      // there's something the rows DON'T say — a second entry, a note to
      ...
      // cadence state — chased recently OR overdue — is derived from the row
      ...
      // Count-based, plainly: ONE entry needs no badge — the row already says
      ...
      // one-entry-with-a-note exception — Hurley restated the rule on count
    ...
    if (rec.is_private) {{
      ...
      var _contactsHtml = pocHtml +
        (_ctp(rec.additional_contacts) ? field('Additional contacts', rec.additional_contacts) : '');
      ...
      var _sibSec = (function () {{
        ...
        var rowsH = use.map(function (x) {{
          return '<div class="sib-line">' +
                 ...
                 '</button>' +
                 // Only hand-linked rows can be unlinked — a shared domain is a
      ...
      // the read — but ABOVE the way out to the organiser's other events.
    ...
    function wireSiblingAdd(rec) {{
      ...
      // Unlink — clear the group on the row that was linked by hand. The
    ...
    if ($side) {{
      ...
      var meIco =
        ...
        'title="' + (_hIn ? "You're interested — click to clear" : "I'm interested") + '" ' +
      ...
      var archIco =
        ...
        'title="' + (_hArch ? 'Archived for you — click to bring it back' : 'Archive — for you only; teammates still see it') + '" ' +
      ...
      $side.querySelectorAll('[data-qa]').forEach(function (b4) {{
        b4.addEventListener('click', window.__qaClick);
      }});
    ...
    // organiser" (Hurley 2026-07-30) — the footer is now only the no-URL note.
...
(function () {{
  ...
  ready(function () {{
    ...
    // Engagement beats scoring. Once we've actually applied, chased, held a
    // meeting, booked or are attending, that event belongs on the named
    // person's list whatever the fit heuristic makes of it — they need to see
    ...
    window.abLiveStage = function (tags) {{
      ...
      // it earned on the way in — 30 of them do. Rejected is terminal and wins
      // over anything else present.
      if (tags.some(function (t) {{ return String(t) === 'Rejected'; }})) return false;
      return tags.some(function (t) {{ return _LIVE_STAGES[String(t)] === 1; }});
    }};
    // Engagement only counts for the people actually ON it — otherwise a
    // submission with nobody named would land in everyone's fits.
    window.abNamedAmong = function (names, meFirst) {{
      if (!meFirst) return false;
    ...
    // The status line is about YOU. Everyone signs in as themselves, so their
    // own name is noise — and a stage that belongs to a COLLEAGUE is not their
    // status at all, so it does not belong on their card (Hurley 2026-07-30).
    // Support is the exception: coordinating the team means seeing all of it.
    //   show  - render this stage at all?
    //   names - what to print after it ('' for a speaker: it is either theirs,
    //           which they already know, or someone else's, which is hidden)
    window.abMineOnly = function (names) {{
      var raw = String(names || '').trim();
      try {{
        if (isSupportPerson(getCollabName() || '')) return {{ show: true, names: raw }};
      }} catch (e) {{ return {{ show: true, names: raw }}; }}
      if (!raw) return {{ show: true, names: '' }};      // nobody named -> the event's own state
      var me = (getCollabName() || '').trim().split(/\\s+/)[0].toLowerCase();
      if (!me) return {{ show: true, names: raw }};
      var mine = raw.split(/\\s*(?:,|;|&| and )\\s*/).some(function (n) {{
        n = String(n || '').trim();
        return n && n.split(/\\s+/)[0].toLowerCase() === me;
      }});
      return {{ show: mine, names: '' }};
    }};
    // Permanent status mark, top-right of every card. Shape says the TRACK
    // (mic = speaking, ticket = attending); fill and colour say the STAGE:
    //   empty + red    rejected      — plus a strike, so it survives being
    //                                  printed, and colour-blind readers
    //   half  + amber  submitted / followed up (in flight)
    //   full  + green  booked
    //   ticket + green attending
    // Drawn as SVG, not emoji: emoji render at different weights and sizes on
    // every OS, and these sit in a row where that shows (Hurley 2026-07-30).
    var _stIconN = 0;
    window.abStageIcon = function (st, ev) {{
      ...
      // own attendance read as nobody attending — HumanX Amsterdam has
    ...
    function _drawStageMark(kind, fill, col, label, pool) {{
      ...
      if (kind === 'ticket') {{
        ...
        // capsule head 9..15 x, 2..13 y; stand + base below
        var head = '<rect x="9" y="2" width="6" height="11" rx="3" fill="none" stroke="' + col + '" stroke-width="1.8"/>';
        var solid = '';
        if (fill === 'full') {{
          solid = '<rect x="9" y="2" width="6" height="11" rx="3" fill="' + col + '"/>';
        }} else if (fill === 'half') {{
          solid = '<clipPath id="' + id + '"><rect x="8" y="7.5" width="8" height="7"/></clipPath>' +
                  '<rect x="9" y="2" width="6" height="11" rx="3" fill="' + col + '" clip-path="url(#' + id + ')"/>';
        }}
        body = solid + head +
          '<path d="M6 11.5a6 6 0 0 0 12 0" fill="none" stroke="' + col + '" stroke-width="1.8" stroke-linecap="round"/>' +
          '<path d="M12 17.5V21" stroke="' + col + '" stroke-width="1.8" stroke-linecap="round"/>';
        if (fill === 'empty') {{
    ...
    function cardStatusLine(ev, st) {{
      ...
      var _spk = window.abMineOnly((st.speaker || '').trim());
      var speaker = _spk.names, _spkShow = _spk.show;
      var _atn = window.abMineOnly(((st.attendees || ev.attendees) || []).map(_personName).filter(Boolean).join(', '));
      var att = _atn.names ? _atn.names.split(', ') : [], _atShow = _atn.show;
        ...
        '<div class="ops-card-head">' +
          '<h3 class="event-name">' +
            (_cardUrl
              ...
              : escapeHtml(ev.name)) +
          '</h3>' +
          // Top-right cluster, in priority order LEFT to RIGHT: the things that
          // only appear on hover sit furthest left, then the permanent status
          // mic, then the message count beside it. Status first, messages
          // second — that's the reading order that matters (Hurley 2026-07-30).
          '<div class="ops-chips">' +
    ...
    function buildManualCard(mev, email) {{
      ...
      var tagsHtml = '';   // status now lives in the corner mark
    ...
    function applyFilterVisibility() {{
      ...
      if ($myTab) {{
        ...
        $myTab.textContent = 'Lineup';   // one name for everyone (Hurley 2026-07-31)
      ...
      opsCards.forEach(function (card) {{
        ...
        if (opsStatFilter) {{
          ...
          if (opsStatFilter === 'myfits') {{
            ...
            // Already in flight for me -> mine, whatever the profile says.
            var _mfLive = window.abLiveStage((card.dataset.statusTags || '').split('|').filter(Boolean)) &&
                          window.abNamedAmong(
                            (card.dataset.interestedNames || '').split('|')
                              .concat((card.dataset.attendeeNames || '').split('|'))
                              .concat([card.dataset.speaker || '']), _attMe);
            // A rejected event is not a fit — the organiser said no. It only
            // stays if I flagged interest myself, since that's my own call to
            // reverse (Hurley 2026-07-30).
            var _mfRej = (card.dataset.statusTags || '').split('|').indexOf('Rejected') !== -1;
            if (_mfRej && !_mfInt) on = false;
    ...
    function renderStats(evs, stateRows, manualRows) {{
      ...
      // The same "already in flight for me" test the filter uses, so the tile
      // number can't drift from the list it opens.
      function _mineRejected(o, st2) {{
        return stageTagsOf(st2 || o).indexOf('Rejected') !== -1;
      }}
      function _mineLive(o, st2, interested) {{
        if (!window.abLiveStage(stageTagsOf(st2 || o))) return false;
        return window.abNamedAmong(
          (interested || []).concat(((st2 && st2.attendees) || o.attendees || []))
            .concat([(st2 && st2.speaker) || o.speaker || '']), meFirst);
      }}
      (evs || []).forEach(function (ev) {{
    ...
    function opsItem(kind, base, st) {{
      ...
      return {{
        ...
        // catalog event invisible to the sibling scan (Hurley 2026-07-31).
        org_group: (st && st.org_group) || base.org_group || '',
    ...
    // Events run by the SAME organiser, for the detail card. The umbrella
    ...
    // Every event carries a SET of organiser keys — its own domain, and any
    // group it was linked into. Two events are siblings when those sets
    // OVERLAP.
    //
    // Matching domain-to-domain and group-to-group separately looked right and
    // was not: linking Money20/20 (money2020.com) into the Terrapinn family
    // stamps org_group on the two events involved, but the OTHER Terrapinn
    // events belong by domain alone and carry no group — so Money20/20 saw only
    // the anchor, and they never saw Money20/20 at all. Overlap closes both
    // directions (Hurley 2026-07-31).
    function _orgKeysOf(o) {{
      var ks = [];
      var d = _urlOrg((o && o.url) || '');
      if (d) ks.push(d);
      var g = String((o && o.org_group) || '').trim();
      if (g && ks.indexOf(g) === -1) ks.push(g);
      return ks;
    }}
    window.abSiblingEvents = function (rec) {{
      var meKeys = _orgKeysOf(rec);
      if (!meKeys.length) return [];
      var mine = String((rec && rec._key) != null ? rec._key : '');
      var mineT = String((rec && rec._table) || '');
      return opsAllItems().filter(function (it) {{
        var o = it.startObj || it;
        // org_group comes off the ITEM (opsItem merges state over base); the url
        // off the underlying row.
        var ks = _orgKeysOf({{ url: o.url || '', org_group: it.org_group || o.org_group || '' }});
        var hit = false;
        for (var i = 0; i < ks.length && !hit; i++) {{
          if (meKeys.indexOf(ks[i]) !== -1) hit = true;
        }}
        if (!hit) return false;
        ...
        var o = it.startObj || it;
        // Linked BY HAND (its group matches mine) rather than by shared domain —
        // only those can be unlinked, since a domain match isn't ours to undo.
        var _g = String(it.org_group || o.org_group || '').trim();
        var byGroup = !!_g && meKeys.indexOf(_g) !== -1 &&
                      _urlOrg(o.url || '') !== _urlOrg((rec && rec.url) || '');
    ...
    window.abOpenRef = function (kind, key) {{ opsOpenRef(kind, key); }};
    ...
    function renderQueue() {{
      ...
      function rowHtml(it, actions) {{
        ...
        return '<div class="queue-row queue-row-open" role="button" tabindex="0"' +
            ...
            // from the planner or Details -> Edit; on a queue row it was an
            // unexplained warning triangle repeated down the whole list. Any
            // conflict already recorded still SHOWS above, as a warning line.
    ...
    function myEventsBuckets() {{
      ...
      // whole team's.
      // Three buckets now (Hurley 2026-07-31): what's coming up for ME, what's
      // coming up for the REST of the team, and everything past. Support has no
      // events of their own, so their "mine" is empty by design and the section
      // is skipped for them.
      var upcoming = [], team = [], past = [];
      ...
      if (named && $opsGrid) {{
        Array.prototype.forEach.call($opsGrid.querySelectorAll('.ops-card'), function (c) {{
          ...
          var spks = (stages.indexOf('Booked') !== -1)
            ? speakerTokens(c.dataset.speaker || r.speaker || '').filter(Boolean) : [];
          // Is anyone from the team on this at all, and is one of them me?
          var everyone = atts.concat(spks).filter(function (x, i, a) {{ return x && a.indexOf(x) === i; }});
          if (!everyone.length) return;
          var iAmOn = !!meFold && everyone.indexOf(meFold) !== -1;
          ...
          var item = {{
            ...
            // Who from the team is on it, for the initials on team rows. Folded
            // names are lower-case, so give them a capital back for the tooltip.
            who: everyone.map(function (n) {{ return n.charAt(0).toUpperCase() + n.slice(1); }})
          }};
          if (_pastFlag) {{
            if (support || iAmOn) past.push(item);
          }} else if (iAmOn) {{
            upcoming.push(item);
          }} else {{
            team.push(item);
        ...
        team.sort(function (a, b) {{ return a.sort - b.sort; }});
        past.sort(function (a, b) {{ return b.sort - a.sort; }});       // most recent first
      }}
      return {{ me: me, support: support, named: named, upcoming: upcoming, team: team, past: past }};
    }}
    ...
    // One definition of "this is an AI question, not a message".
    window.abIsAskRow = function (body) {{
      return /^\s*\u2753\s*Asked AI:/.test(String(body || ''));
    }};
    function _askText(body) {{
      return String(body || '').replace(/^\s*\u2753\s*Asked AI:\s*/, '').trim();
    }}
    function _whatsNewItems(all) {{
      ...
      Object.keys(_chatMeta || {{}}).forEach(function (k) {{
        ...
        var freshAll = (meta.msgs || []).filter(function (x) {{ return x.at > last && String(x.author || '').toLowerCase().split(/\\s+/)[0] !== me; }});
        var kind = k.charAt(0) === 'm' ? 'manual' : 'catalog'; var key = k.slice(1);
        // A question to the assistant is NOT a comment. It's a signal of
        // interest, so support gets it \u2014 bunched per person like routine
        // updates, never a card each (Hurley 2026-07-30). Nobody else sees
        // them; they'd only be reading their own questions back.
        var _asks = freshAll.filter(function (x) {{ return window.abIsAskRow(x.body); }});
        var fresh = freshAll.filter(function (x) {{ return !window.abIsAskRow(x.body); }});
        var _arec = (k.charAt(0) === 'm' ? byMid[k.slice(1)] : byNum[k.slice(1)]);
        if (_wnSupport && _asks.length && _arec) {{
          _asks.forEach(function (x) {{
            var whoA = String(x.author || 'Someone').split(/\\s+/)[0];
            var idA = 'q:' + k + ':' + x.at;
            if (dis[idA]) return;
            items.push({{ id: idA, ts: x.at, kind: kind, key: key, type: 'update',
              who: whoA, ask: true,
              detail: 'asked AI about ' + (_arec.name || 'an event') +
                      ' \u2014 \u201c' + _askText(x.body) + '\u201d',
              label: whoA + ' asked AI about ' + (_arec.name || 'an event') }});
          }});
        }}
    ...
    function renderMyEvents() {{
      ...
      function section(title, list, emptyMsg, collapsible, collapsed, showWho) {{
        ...
        var body = list.length ? list.map(function (it) {{ return rowHtml(it, showWho); }}).join('')
      ...
      // Named to say WHOSE events these are, with the team's set directly
      // below (Hurley 2026-07-31).
      var upTitle = 'My Upcoming Events';
              ...
              '<div class="wn-group-head" role="button" tabindex="0">' +
                ...
                '<span class="wn-group-count">' + list.length + ' ' +
                  (list.every(function (o) {{ return o.w.ask; }})
                    ? ('question' + (list.length === 1 ? '' : 's') + ' to AI')
                    : ('routine update' + (list.length === 1 ? '' : 's'))) + '</span>' +
    ...
    function followUpState(o, stages, extraNotes) {{
      ...
      // A rejection is a different thing from a door that was never open —
      // "they said no" earns the red dot; sponsorship-only or not-taking-
      // speakers is structural, and stays grey (Hurley 2026-07-30).
      if (shutWhy) return {{ state: 'closed', rejected: (shutWhy === 'they said no'),
                            label: 'Closed \u2014 ' + shutWhy }};
      ...
      var org = _fuOrgKey(o), _via = null;
      if (org) {{
        var shared = (_fuByOrg || _buildFollowUpOrgIndex())[org];
        if (shared && shared.on && (!lastOn || String(shared.on) > String(lastOn))) {{
          lastOn = shared.on;
          // Remember WHOSE chase this was — a card with no log of its own can
          // then show the row rather than a bare label (Hurley 2026-07-30).
          _via = {{ on: shared.on, name: shared.name }};
        }}
      }}
      if (!lastOn) return {{ state: 'none', label: 'Not contacted yet' }};
      var days = _fuDaysSince(lastOn, today);
      if (days >= followUpEvery(o, today)) {{
        return {{ state: 'due', days: days, since: lastOn, via: _via,
                 label: 'Follow up now \u2014 ' + days + ' days since ' + _fuNice(lastOn) }};
      }}
      return {{ state: 'ok', days: days, since: lastOn, via: _via,
               label: 'Followed up ' + _fuNice(lastOn) }};
    }}
    ...
    // Questions to the assistant are transient — they exist to tell Angela what
    // someone was looking at this week, not to become permanent history. Once
    // they age past the "In the last week" window they have served their
    // purpose, so they're cleared rather than piling up forever
    // (Hurley 2026-07-30). Support runs the sweep so it happens once, not once
    // per teammate.
    function _purgeOldAsks() {{
      if (!isSupportPerson(getCollabName() || '')) return;
      var cut = new Date(Date.now() - 7 * 86400000).toISOString();
      try {{
        sb.from('event_chat').delete()
          .lt('created_at', cut).like('body', '%Asked AI:%')
    ...
    function _paintChatList(list, msgs) {{
      ...
      document.querySelectorAll('.chat-fwd-menu, .chat-more-menu').forEach(function (x) {{ x.remove(); }});
      list.innerHTML = '';
      // "Asked AI" rows are never conversation, so they never render in the
      // thread \u2014 for anyone. They reach Angela as a bunched row in "In the
      // last week" instead (Hurley 2026-07-30).
      msgs = msgs.filter(function (m) {{ return !window.abIsAskRow(m.body); }});
