...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* Edit-form zones: a rule + small caps heading separating Basics / Notes /
       Speaking / Attending / Contacts / Details, so the form is scannable
       instead of one 30-field column (Angela). */
    .me-sec {{ margin-top: 22px; padding-top: 14px; border-top: 1px solid var(--ab-rule); }}
    .me-sec:first-of-type {{ margin-top: 0; padding-top: 0; border-top: 0; }}
    .me-sec-h {{
      margin: 0 0 12px; font-family: var(--ab-sans); font-size: 0.72rem; font-weight: 600;
      letter-spacing: 0.08em; text-transform: uppercase; color: var(--ab-fg-3);
    }}
...
(function () {{
  ...
  function editFormHtml(rec) {{
    ...
    if (!priv) {{
      ...
      sDet += ef('About', ta('about', rec.about));
