...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .profile-who {{ font-family: var(--ab-sans); font-weight: 700; font-size: 1.05rem; color: var(--ab-fg); }}
...
(function () {{
  ...
  ready(function () {{
    ...
    function renderMyProfile() {{
      ...
      if (!meName) {{
        host.innerHTML = '<div class="profile-wrap"><p class="queue-intro myev-intro"><strong>My Profile.</strong> Set your name (hit &ldquo;change&rdquo; up top) to build your speaker profile &mdash; bio, topics, past talks, files, and the events you want to target.</p></div>';
        return;
      }}
      var meKey = _profileKey(meName);
      host.innerHTML = '<div class="profile-wrap"><p class="queue-intro myev-intro">Loading your profile&hellip;</p></div>';
      sb.from('team_profiles').select('*').then(function (resp) {{
        var dbMissing = !!(resp && resp.error);
        var rows = (resp && resp.data) || [];
        var byKey = {{}};
        rows.forEach(function (r) {{ byKey[r.person] = r; }});
        _paintMyProfile(host, meKey, meName, byKey[meKey] || null, rows, dbMissing);
      }});
    }}
    function _paintMyProfile(host, meKey, meName, myRow, allRows, dbMissing) {{
      ...
      if (dbMissing) {{
        html += '<div class="profile-setup-note"><strong>One-time setup needed.</strong> The profile store isn&#39;t in the database yet, so nothing here will save. Whoever runs the tracker needs to run the <code>team_profiles</code> setup once, then reload.</div>';
      }}
      html += '<div class="profile-card"><div class="profile-card-head">' +
        '<span class="profile-avatar">' + initial + '</span>' +
        '<span class="profile-who">' + escapeHtml(disp) + '</span></div>';
      ...
      html += '<div class="profile-field" style="margin-top:20px;"><label>Files <span class="hint">&middot; bios, decks, one-pagers</span></label>' +
        '<div class="profile-files" id="pf-files"><p class="profile-file-empty">Loading files&hellip;</p></div>' +
        '<div class="profile-upload-row"><input type="file" id="pf-file-input" aria-label="Choose a file to upload"><button type="button" class="q-btn" id="pf-upload">Upload</button></div></div>';
      html += '</div>';   // end .profile-card
      var mates = (allRows || []).filter(function (r) {{ return r.person !== meKey && (r.bio || r.topics || r.past_talks || r.notes); }});
      if (mates.length) {{
        mates.sort(function (a, b) {{ return _profileDisplay(a, a.person).localeCompare(_profileDisplay(b, b.person)); }});
        html += '<div class="queue-sec-head" style="margin-top:8px;"><span class="queue-sec-title">The rest of the team</span><span class="queue-sec-count">' + mates.length + '</span></div>' +
          '<p class="queue-meta" style="margin:-4px 0 14px;">Read-only &mdash; where each person has spoken and what they want to target.</p>';
        mates.forEach(function (r) {{
          var d = _profileDisplay(r, r.person);
          html += '<div class="profile-teammate"><div class="profile-tm-head"><span class="profile-avatar">' + escapeHtml((d || '?').charAt(0).toUpperCase()) + '</span><span class="profile-tm-name">' + escapeHtml(d) + '</span></div>';
          PROFILE_FIELDS.forEach(function (f) {{
            if (!r[f.k]) return;
            html += '<div class="profile-tm-field"><div class="k">' + escapeHtml(f.label) + '</div><div class="v">' + escapeHtml(r[f.k]) + '</div></div>';
          }});
          html += '</div>';
        }});
      }}
