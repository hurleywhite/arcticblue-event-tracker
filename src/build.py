  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .tf-count {{
      ...
      display: inline-flex; align-items: center; justify-content: center;
      font-family: var(--ab-mono); font-size: 0.66rem; font-weight: 700; line-height: 1;
      color: #fff; background: var(--ab-blue); border-radius: 999px;
    }}
      ...
      <div id="angela-ops" hidden>
        ...
        <div class="ops-topfilters" id="ops-topfilters">
          ...
          <button type="button" class="tf-toggle" id="ops-filter-toggle" aria-haspopup="true" aria-expanded="false" aria-controls="tf-drawer" title="More filters — region, months, pipeline and more" aria-label="More filters">
            ...
            <span class="tf-count" id="tf-active-count" hidden></span>
          </button>
  ...
  ready(function () {{
    ...
    function cardStatusLine(ev, st) {{
      ...
      if (stages.indexOf('Booked') !== -1) {{
        ...
        bits.push({{ p: 3, h: '<span class="st-bit"><span class="st-dot st-ok"></span>Open to speak</span>' }});
      }}
    ...
    function updateFilterToggle() {{
      ...
      if (!$ft) return;
      // Every non-status filter (region/months/pipeline/price/… + the checkboxes
      // and search) now lives behind this one icon — badge the count so an
      // engaged filter is never invisible.
      ...
      if (named && $opsGrid) {{
        Array.prototype.forEach.call($opsGrid.querySelectorAll('.ops-card'), function (c) {{
          ...
          var item = {{
            ...
            is_private: !!r.is_private   // private events carry no public info to brief from
          }};
    ...
    function renderMyEvents() {{
      ...
      function rowHtml(it) {{
        ...
        // PRIVATE event has no public info to brief from, so no brief button.
        var brief = (it._past || it.is_private) ? '' :
