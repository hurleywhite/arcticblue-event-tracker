    ...
    <div class="panel" id="panel-angela" role="tabpanel" data-tab="angela" aria-labelledby="tab-angela">
      ...
      <div id="angela-ops" hidden>
        ...
        <div class="ops-filters">
          ...
          <label class="ops-filter-chip" title="Show only events marked No-go, to review or restore them"><input type="checkbox" id="ops-f-nogo">Passed (no-go)</label>
...
(function () {{
  ...
  ready(function () {{
    ...
    function applyFilters() {{
      ...
      var $nogo    = document.getElementById('ops-f-nogo');
      ...
      var fNogo      = !!($nogo && $nogo.checked);
      // Toggle has-active classes for chip styling
      [['ops-f-saved',$saved],['ops-f-urgent',$urgent],['ops-f-speaker',$speaker],['ops-f-buyers',$buyers],['ops-f-meetings',$meet],['ops-f-worth',$worth],['ops-f-past',$past],['ops-f-hidden',$hidden],['ops-f-nogo',$nogo]].forEach(function (pair) {{
      ...
      $opsGrid.querySelectorAll('.ops-card').forEach(function (card) {{
        ...
        if (fNogo && card.dataset.decision !== 'no-go') on = false;
    ...
    function wireFilters() {{
      ['ops-search','ops-region','ops-price','ops-fits','ops-f-saved','ops-f-urgent','ops-f-speaker','ops-f-buyers','ops-f-meetings','ops-f-worth','ops-f-past','ops-f-hidden','ops-f-nogo'].forEach(function (id) {{
    ...
    function clearOpsFilters() {{
      ...
      ['ops-f-saved','ops-f-urgent','ops-f-speaker','ops-f-buyers','ops-f-meetings','ops-f-worth','ops-f-past','ops-f-hidden','ops-f-nogo'].forEach(function (id) {{
    ...
    function openBriefDrawer(kind, key) {{
      ...
      if (!ov) {{
        ...
        var _closeBf = function () {{ ov.classList.remove('show'); document.body.style.overflow = ''; }};
        ov.addEventListener('click', function (e) {{ if (e.target === ov) _closeBf(); }});
        ov.querySelector('.bf-close').addEventListener('click', _closeBf);
        document.addEventListener('keydown', function (e) {{ if (e.key === 'Escape' && ov.classList.contains('show')) _closeBf(); }});
      }}
      ov._it = it;
