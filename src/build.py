  ...
  ready(function () {{
    ...
    function _chatReactLine(rx, me) {{
      ...
      Object.keys(rx).forEach(function (em) {{
        ...
        out.push('<span class="chat-react' + (mine ? ' is-mine' : '') + '" title="' + escapeHtml(list.join(', ')) + '">' + em + ' ' + list.length + '</span>');
      }});
    ...
    function _chatReact(m, emoji) {{
      ...
      sb.from('event_chat').update({{ reactions: rx }}).eq('id', m.id).then(function (resp) {{
        if (resp.error) {{
