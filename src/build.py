...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* Cluster-style pins: a colored badge with the event count. */
    .map-pin {{
...
(function () {{
  ...
  ready(function () {{
    ...
    // Resolve the stage list for an ops/manual row: prefer the stored
    // status_tags array; fall back to deriving from the legacy status.
    function stageTagsOf(st) {{
      if (!st) return [];
      var norm = normalizeStageTags(st.status_tags);
      if (norm.length) return norm;
      return legacyToStages(st.status);
    }}
    ...
    function renderOpsMap() {{
      ...
      Object.keys(byCoord).forEach(function (key) {{
        ...
        // Cluster-style badge: a colored dot for one event, a count badge for
        // several. Sized gently with the count so big hubs read as big.
        var size = n === 1 ? 14 : Math.min(26 + n * 1.2, 44);
        var icon = L.divIcon({{
          className: '',  // suppress Leaflet's default white square
          html: '<div class="map-pin' + (n === 1 ? ' single' : '') +
                '" style="background:' + color + '">' + (n === 1 ? '' : n) + '</div>',
          iconSize: [size, size],
