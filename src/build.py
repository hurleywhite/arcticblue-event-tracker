...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* Search sits inline with the filters — a moderate, fixed-ish width so it
       doesn't stretch across the whole row. */
    ...
    /* Row 2: view tabs, then the add-event actions grouped right beside them
       (left-aligned — no awkward gap stretched across the row). */
    .ops-controls-row {{
      display: flex; flex-wrap: wrap; align-items: center;
      justify-content: flex-start; gap: 16px; margin-bottom: 16px;
    }}
  ...
  ready(function () {{
    ...
    function buildMonthsFilter() {{
      ...
      months.forEach(function (m) {{
        var chip = _makeExtraChip(m.key, m.label, 'month-chip');
    ...
    function openSearchPanel(email) {{
      ...
      panel.querySelector('#search-run-btn').addEventListener('click', function () {{
        ...
        var criteria = {{ count: count, types: SEARCH_TYPE_OPTIONS.slice(), quarters: getQuarters(), regions: getRegions() }};
