...
(function () {{
  ...
  function briefClause(v) {{
    ...
    // overview. Give it a verb (Hurley 2026-07-30). Anything already written
    // as a sentence is left alone.
    if (!/\\b(is|are|was|were|include|includes|offer|offers|feature|features|run|runs|host|hosts|provide|provides|guarantee|guarantees|use|uses|has|have|allow|allows|unlock|unlocks|cover|covers|grant|grants|combine|combines|admit|admits|give|gives|bring|brings|enable|enables|let|lets|mean|means|apply|applies|work|works|take|takes|require|requires|consist|consists|comprise|comprises|span|spans|award|awards|pair|pairs|connect|connects)\\b/i.test(pick)) {{
      var _lst = pick.replace(/,\\s*([^,]+)$/, ' and $1');       // last comma -> "and"
      // Don't lowercase a brand — "Reuters Events app" must not become
      // "reuters". A second capitalised word is the tell that we're in a
      // proper name rather than an ordinary noun phrase.
      if (!/^[A-Z][a-z]+.*\\s[A-Z][a-z]/.test(_lst)) {{
        _lst = _lst.charAt(0).toLowerCase() + _lst.slice(1);
      }}
      // "includes attendee app for pre-booking" wants an article. The head noun
      // sits before the first preposition, so test THAT for a plural rather
      // than the last word of the whole phrase.
      var _cut = _lst.search(/\\s(?:for|with|via|in|on|at|to|during|across|from|and)\\s|,/i);
      var _head = (_cut > 0 ? _lst.slice(0, _cut) : _lst).trim().split(/\\s+/).pop()
                    .replace(/[^A-Za-z-]+$/, '');            // drop trailing "(", ")", ":"
  ...
  function wireQuickBar(rec) {{
    ...
    window.__qaClick = function (e) {{
      ...
      if (qa === 'saved') {{ rec.saved = !rec.saved; patch.saved = rec.saved; }}
        ...
        // Marking an OUTCOME retires the application state, so the stored row
        // matches what everyone reads (Hurley 2026-07-30). Turning the outcome
        // back OFF doesn't restore Submitted — that'd be guessing at history;
        // the button is right there if it should come back.
        if ((stage === 'Booked' || stage === 'Rejected') && idx === -1) {{
          var _sub = tags.indexOf('Submitted');
          if (_sub !== -1) tags.splice(_sub, 1);
        }}
...
(function () {{
  ...
  ready(function () {{
    ...
    window.abStageIcon = function (st, ev) {{
      ...
      var att = window.abMineOnly((((st && st.attendees) || (ev && ev.attendees) || [])
                  .map(function (a) {{ return String(a || ''); }}).join(', ')));
      var kind = null, fill = 'none', col = '', label = '';
      if (who.show && tags.indexOf('Booked') !== -1) {{
        kind = 'mic'; fill = 'full';  col = '#15803d'; label = 'Booked';
      }} else if (who.show && tags.indexOf('Rejected') !== -1) {{
        kind = 'mic'; fill = 'empty'; col = '#b91c1c'; label = 'Rejected';
      }} else if (who.show && (tags.indexOf('Submitted') !== -1 || tags.indexOf('Followed up') !== -1 || tags.indexOf('Meeting held') !== -1)) {{
        kind = 'mic'; fill = 'half';  col = '#b45309'; label = 'Submitted';
      }} else if (att.show && (att.names || tags.indexOf('Attending') !== -1)) {{
        kind = 'ticket'; col = '#15803d'; label = 'Attending';
      }}
      if (!kind) return '';
      var id = 'stq' + (++_stIconN);
      var body;
      ...
      return '<span class="ops-stage-ico" title="' + escapeHtml(label) + '" aria-label="' + escapeHtml(label) + '" role="img">' +
             '<svg viewBox="0 0 24 24" aria-hidden="true">' + body + '</svg></span>';
    }};
