  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .view-toggle {{
      ...
      max-width: 100%; padding: 3px; background: var(--ab-bg-3);
...
{archived_html}
    ...
    <div class="panel" id="panel-angela" role="tabpanel" data-tab="angela" aria-labelledby="tab-angela">
      ...
      <div id="angela-ops" hidden>
        ...
        <div class="stage-filters region-filters" id="region-filters">
          <span class="label">Region:</span>
          <!-- region chips injected by buildRegionFilters() -->
        </div>
...
(function () {{
  ...
  function quickBarHtml(rec) {{
    ...
    var summary = formatInterested(window.visibleInterested(rec.interested, rec.speaker, rec.attendees));
    var intRow =
  ...
  function wireQuickBar(rec) {{
    ...
    bar.addEventListener('click', function (e) {{
      ...
      if (qa === 'saved') {{ rec.saved = !rec.saved; patch.saved = rec.saved; }}
        ...
        if (hit === -1) ilist.push(me); else ilist.splice(hit, 1);
...
(function () {{
  ...
  ready(function () {{
    ...
    function buildOpsCard(ev, st, email) {{
      ...
      var contactBadge = card.dataset.contactFound === '1' ? '<span class="contact-badge" title="An email contact was found for this event">✉ Contact</span>' : '';
      ...
      if (st) {{
        ...
        ['why', 'about', 'focus_areas', 'typical_attendees', 'speaking_route',
         ...
         'url', 'pricing', 'audience_type', 'past_speakers',
    ...
    function buildManualCard(mev, email) {{
      ...
      var mContactBadge = card.dataset.contactFound === '1' ? '<span class="contact-badge" title="An email contact was found for this event">✉ Contact</span>' : '';
    ...
    function buildRegionFilters() {{
      var host = document.getElementById('region-filters');
      if (!host || host.dataset.built === '1') return;
      var frag = document.createDocumentFragment();
      Object.keys(REGION_COLORS).forEach(function (r) {{
        var col = REGION_COLORS[r];
        var btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'stage-chip region-chip';
        btn.dataset.region = r;
        btn.setAttribute('aria-pressed', 'false');
        btn.style.background = hexToRgba(col, 0.14);
        btn.style.color = col;
    ...
    function applyFilters() {{
      ...
      [['ops-f-saved',$saved],['ops-f-urgent',$urgent],['ops-f-submitted',$submitted],['ops-f-meetings',$meet],['ops-f-worth',$worth],['ops-f-aipicks',$aipicks],['ops-f-contact',$contact],['ops-f-past',$past],['ops-f-hidden',$hidden],['ops-f-recent',$recent]].forEach(function (pair) {{
