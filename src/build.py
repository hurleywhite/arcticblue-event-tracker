...
(function () {{
  ...
  ready(function () {{
    ...
    function buildOpsCard(ev, st, email) {{
      ...
      // Priority (High/Medium/Low) is intentionally NOT shown on the card face
      // anymore — open the event to see it in the pop-up.
      var sigBits = [];
      ...
      // Deadline/closed-to-speak label + Apply sit side by side in one row.
      var _cDeadlineHtml = deadlineLine(ev.deadline, ev);
      var _cFootHtml = (_cDeadlineHtml || applyBtn) ? ('<div class="ops-card-foot">' + _cDeadlineHtml + applyBtn + '</div>') : '';
      card.innerHTML =
    ...
    function buildManualCard(mev, email) {{
      ...
      var _mFootHtml = (_mDeadlineHtml || mApplyBtn) ? ('<div class="ops-card-foot">' + _mDeadlineHtml + mApplyBtn + '</div>') : '';
