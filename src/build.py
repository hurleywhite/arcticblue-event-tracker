...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* "Closed to speak" — CFP window has passed (rectangular label). */
    .closed-to-speak {{
      display: inline-block; font-family: var(--ab-mono); font-size: 0.6rem; font-weight: 700;
      letter-spacing: 0.06em; text-transform: uppercase; padding: 3px 8px; border-radius: 3px;
      background: #fee2e2; color: #991b1b; border: 1px solid #f3b1b1;
    }}
    ...
    .ops-card .event-name {{
      ...
      margin: 0 0 4px; color: var(--ab-fg); overflow-wrap: anywhere;
    }}
    .ops-card .event-loc {{
    ...
    /* Per-role roster on the card face — aligned "Stage: who" rows. */
    .ops-roster {{ display: flex; flex-direction: column; gap: 5px; margin: 8px 0 8px; }}
    .ops-roster-row {{ display: flex; align-items: center; gap: 9px; font-size: 0.86rem; line-height: 1.3; }}
    .ops-roster-tag {{
    ...
    .ops-tag.pri-high   {{ background: #166534; color: #fff; }}
    ...
    .ops-myevents, .ops-queue, .ops-planner, .ops-dayof {{ display: none; }}
    .ops-myevents.show, .ops-queue.show, .ops-planner.show, .ops-dayof.show {{ display: block; }}
    /* ── Day-Of tab ─────────────────────────────────────────────── */
...
<body class="hide-urgent">
  ...
  <nav class="nav">
    <div class="nav-inner">
      ...
      <div class="nav-meta">{last_updated.upper()} <span class="who">· Signed in as <strong id="ops-email">Team</strong> <button type="button" id="change-name" class="inline">change</button></span></div>
    </div>
  </nav>
...
(function () {{
  ...
  ready(function () {{
    ...
    function _rosterRow(label, who, style, muted) {{
      // Pipeline-stage rows carry their stage colour so they MATCH the Pipeline
      // filter chips (Submitted = blue, Booked = green, etc.). Non-stage rows
      // (Speaker/Interested) pass style='' and stay the neutral grey label.
      var attr = style ? ' style="' + style + '"' : '';
      return '<div class="ops-roster-row">' +
        '<span class="ops-roster-tag"' + attr + '>' + escapeHtml(label) + '</span>' +
    ...
    function renderOpsRoster(st, extra) {{
      ...
      if (spStage) rows.push(_rosterRow(spStage, speaker ? escapeHtml(speaker) : '&mdash;', stageStyle(spStage), !speaker));
      else if (speaker) rows.push(_rosterRow('Speaker', escapeHtml(speaker), ''));
    ...
    function deadlineLine(d, o) {{
      ...
      if (isDeadlinePast(d)) {{
        // CFP/apply window has closed. On events that HAVEN'T happened yet, show a
        // "Closed to speak" label (you can still attend, just not apply to speak).
        return (typeof isPastEvent === 'function' && isPastEvent(o)) ? '' :
          '<p class="ops-meta"><span class="closed-to-speak" title="The apply-to-speak / CFP window has closed">Closed to speak</span></p>';
      }}
      var txt = String(d).trim();
    ...
    function buildManualCard(mev, email) {{
      ...
      var _mPri = cardPriority(mev);
      var audChip = _mPri ? '<span class="ops-tag pri-' + _mPri.toLowerCase() + '">' + escapeHtml(_mPri) + '</span>' : '';
