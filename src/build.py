...
(function () {{
  ...
  function openEventModal(rec) {{
    ...
    var v = '';
    // Notes lead the read view — right below "Chat with the team" (Angela).
    v += sec('Notes', field('Notes', rec.notes));
    if (rec.is_private) {{
      ...
      v += sec('Who attends',
        field('Typical attendees', rec.typical_attendees) +
        field('Past / announced speakers', rec.past_speakers) +
        (function () {{
          var g = field('Attendee count', rec.attendee_count) + field('Audience', rec.audience_type);
          return g ? '<div class="modal-grid">' + g + '</div>' : '';
        }})() +
        field('Meetings & networking', rec.meeting_formats));
      // — About the event itself.
      v += sec('About the event',
