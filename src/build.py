  ...
  function editFormHtml(rec) {{
    ...
    if (!isCat) {{
      h += '<div class="me-danger"><button type="button" class="me-delete">Delete this event</button>' +
           '<span class="me-danger-note">Removes this manually-added event. Cannot be undone.</span></div>';
    }}
    return h;
  }}
...
(function () {{
  ...
  ready(function () {{
    ...
    // Delete bridge for the modal's Edit form (manual events only).
    window.opsDelete = function (table, key) {{
      if (table !== 'manual_events') return Promise.resolve({{ error: {{ message: 'Only manual events can be deleted.' }} }});
