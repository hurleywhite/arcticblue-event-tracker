    ...
    window.opsRenderChat = function (rec) {{
      ...
      if (a1) a1.addEventListener('submit', function (e) {{
        ...
        fetch('/api/ask', {{
          ...
          // Recorded, but NOT as a chat message — it never renders in the
          // thread. It surfaces once, bunched per person, in Angela's "In the
          // last week" ("Thor \u00b7 3 questions to AI"), so she can see what
          // he's circling without wading through each one (Hurley 2026-07-30).
          var _askRow = {{ author: who, body: '\u2753 Asked AI: ' + q }};
          _askRow[col] = rec._key;
          sb.from('event_chat').insert(_askRow).then(function () {{}}, function () {{}});
