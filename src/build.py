...
(function () {{
  ...
  function quickBarHtml(rec) {{
    ...
    return '<div class="modal-quickbar">' + b.join('') + '</div>';
  }}
  ...
  function openEventModal(rec) {{
    ...
    }} else if (rec._table && rec._key != null) {{
      actHtml += '<button type="button" class="modal-edit-btn" id="modal-add-link">+ Add a website link</button>';
    }} else {{
      actHtml += '<span class="modal-nolink">No verified website URL on file.</span>';
    }}
    $actions.innerHTML = actHtml;
    var editBtn = document.getElementById('modal-edit-event');
    if (editBtn) editBtn.addEventListener('click', function () {{ if (window.opsOpenEditor) window.opsOpenEditor(rec._table, rec._key); }});
    var addLinkBtn = document.getElementById('modal-add-link');
    if (addLinkBtn) addLinkBtn.addEventListener('click', function () {{ if (window.opsOpenEditor) window.opsOpenEditor(rec._table, rec._key); }});
