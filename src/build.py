...
(function () {{
  ...
  ready(function () {{
    ...
    var STAGE_DISPLAY_RANK = ['Booked', 'Attending', 'Declined', 'Meeting held', 'Submitted', 'Identified'];
    ...
    function buildOpsCard(ev, st, email) {{
      ...
      if (st) {{
        ...
        ['why', 'about', 'focus_areas', 'typical_attendees', 'speaking_route',
         'pay_to_play', 'venue', 'contact_info', 'deadline',
      ...
      function section(title, list, kind) {{
        ...
        var rows = list.map(function (it) {{
          return rowHtml(it, function (x) {{
            ...
            btns += '<button class="q-btn" data-ref-kind="' + x.kind + '" data-ref-key="' + escapeHtml(String(x.key)) + '">Details &rarr;</button>';
            return btns;
          }});
        }}).join('');
    ...
    function renderOps(email) {{
      ...
      return Promise.all([
        ...
        // Drop catalog events that were soft-deleted (a persistent sentinel on
    ...
    var _STAGE_ORDER = ['Identified', 'Submitted', 'Meeting held', 'Booked', 'Attending', 'Declined'];
