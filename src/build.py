...
(function () {{
  ...
  ready(function () {{
    ...
    function extractFromEmail(text) {{
      ...
      if (subj) {{
        out.name = subj[1].replace(/^(?:Re:|Fwd?:|FW:)\s*/i, '').trim();
      }} else {{
      ...
      // Location — keyword lines first; else "in/at City"
      var lk = text.match(/^\s*(?:Location|Where|Venue|City)\s*:\s*(.+)$/im);
      if (lk) {{
        out.location = lk[1].trim();
      }} else {{
        var lc = text.match(/\\b(?:in|at)\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){{0,2}}(?:,\s+[A-Z][a-zA-Z]+)?)/);
    ...
    function deriveStartDateFromText(text) {{
      ...
      var m = String(text).match(/([A-Za-z]+)\\s+(\\d{{1,2}})[^\\d]*,?\\s*(\\d{{4}})/);
      if (!m) return null;
